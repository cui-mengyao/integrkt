...
from modeling.Med_SAM.prompt_encoder import TaskPromptEncoder_Subtype as ClsPromptEncoder
...
def main():
    ...
    feature_opt = AdamW([i for i in cls_prompt_encoder.parameters() if i.requires_grad == True], lr=args.lr, weight_decay=0)
    feature_scheduler = torch.optim.lr_scheduler.LinearLR(feature_opt, start_factor=1.0, end_factor=0.01, total_iters=500)

    img_encoder.eval()
    prompt_encoder.eval()
    mask_decoder.eval()
    cls_prompt_encoder.train()
    ...
    for epoch_num in range(args.max_epoch):
        ...
        for idx, (img, seg, spacing, cls_label) in enumerate(train_data):
          ...
          for i in range(6):
               head = cls_prompt_encoder(feature_list[3], i)
               masks = mask_decoder_2(middle_feature, head, img_resize, patch_size//64)
               masks = masks.permute(0, 1, 4, 2, 3)
               if cls_label == i:
                  loss += loss_cal(masks, seg)
               elif cls_label != i :
                  loss += loss_cal(masks, blank)/5.0
          ...
          torch.nn.utils.clip_grad_norm_(cls_prompt_encoder.parameters(), 1.0)
        ...
        save_checkpoint({"epoch": epoch_num,
                         "cls_prompt_dict": cls_prompt_encoder.state_dict(),
