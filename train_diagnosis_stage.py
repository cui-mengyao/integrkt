...
from modeling.Med_SAM.prompt_encoder import TaskPromptEncoder_Stage as ClsPromptEncoder_2
...
def main():
    ...
    cls_model = ClassNet(in_channels=640+128, num_classes=1).to(device)
    ...
    for epoch in range(args.max_epoch):    
        ...
        if target_class == 2:
           ...
           print('loss vl:', epoch, loss_avg[0], 'acc:', acc, 'auc:', auc, "sen:", sen, "spe:", spe)
           print(conf_mat)
        ...
        with open(args.snapshot_path_2+'result.txt', 'a') as file:
            np.savetxt(file, [epoch, auc, acc], fmt='%f', delimiter=',')
