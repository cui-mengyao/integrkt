...
class Cls_Decoder(nn.Module):
    def __init__(self, mlahead_channels=128, num_classes=2,**kwargs):
        super(Cls_Decoder, self).__init__(**kwargs)
        self.mlahead_channels = mlahead_channels
        self.cls = nn.Sequential(nn.Conv3d(5 * mlahead_channels + 1, mlahead_channels, 3, padding=1, bias=False),
                     nn.InstanceNorm3d(mlahead_channels),
                     nn.ReLU(),
                     nn.Conv3d(mlahead_channels, num_classes, 3, padding=1, bias=False))

    def forward(self, x, y, z, scale_factor2):
