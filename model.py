...
class ClassNet(nn.Module):
    def __init__(self, in_channels=128, base_channels=128, num_classes=5, ):
        ...
        self.relu = ReLU()
        self.dropout = Dropout(p=0.2, inplace=False)
...
class ClassNet_Linear(nn.Module):
    def __init__(self, in_channels=128, base_channels=128, num_classes=5, ):
        super(ClassNet_Linear, self).__init__()
        self.fc1 = Linear(in_channels, base_channels)
        self.fc1_2 = Linear(in_channels, base_channels)
        self.fc2 = Linear(base_channels, base_channels)
        self.fc3 = Linear(base_channels, num_classes)
        self.relu = ReLU()
        #self.sig = Sigmoid()
...
class ClassNet_channel(nn.Module):
    def __init__(self, in_channels=128, base_channels=128, num_classes=5, ):
        super(ClassNet_channel, self).__init__()
        self.fc1 = Linear(in_channels, in_channels)
        self.fc2 = Linear(in_channels, base_channels)
        self.fc3 = Linear(base_channels, num_classes)
        self.relu = ReLU()
        self.dropout = Dropout(p=0.2, inplace=False)
...
class ClassNet_channel_v2(nn.Module):
    def __init__(self, in_channels=128, base_channels=128, num_classes=5, ):
        super(ClassNet_channel_v2, self).__init__()
        self.fc1 = Linear(in_channels, in_channels)
        self.fc2 = Linear(in_channels, base_channels)
        self.fc3 = Linear(base_channels, num_classes)
        self.relu = ReLU()
    ...
    def forward(self, x, y):
        ...
        attn = self.relu(attn)
        attn_2 = self.fc1(y)
        attn_2 = self.relu(attn_2)
        attn = self.sig(attn + attn_2)
        x = x * attn
        x = self.relu(x)
        x = self.dropout(x)        
        x = self.fc2(x)
...
class ClassNet_channel_v3(nn.Module):
    def __init__(self, in_channels=128, base_channels=128, num_classes=5, ):
        super(ClassNet_channel_v3, self).__init__()
        self.fc1 = Linear(in_channels, base_channels)
        self.fc2 = Linear(base_channels, base_channels)
        self.fc3 = Linear(base_channels, num_classes)
        self.relu = ReLU()
...
class ClassNet_early_merge(nn.Module):
    def __init__(self, in_channels=128, base_channels=128, num_classes=5, ):
        super(ClassNet_early_merge, self).__init__()
        self.conv = ConvBlock(in_channels, base_channels)
        self.fc2 = Linear(base_channels, base_channels)
        self.fc3 = Linear(base_channels, num_classes)
        self.relu = ReLU()
        self.sig = Sigmoid()
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x, y=None, use_seg=True, get_seg=False):
        ...
        if y is not None:
            ...
            area = torch.sum(mask.view(mask.size(0), -1), dim=1, keepdim=True)
            masked = x * mask
            print("3:",masked.shape)
            masked = torch.sum(masked.view(masked.size(0), masked.size(1), -1), axis=2)
            masked = masked / area
            print("4:",masked.shape)
