...
from modeling.Med_SAM.prompt_encoder import TaskPromptEncoder_Malignancy as ClsPromptEncoder
...
def main():
    ...
    cls_model = ClassNet(in_channels=640, num_classes=1).to(device)

    ce_loss = CrossEntropyLoss()
    optimizer_cls = Adam(cls_model.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.LinearLR(optimizer_cls, start_factor=1.0, end_factor=0.01, total_iters=500)
    ...
    def model_predict(img, prompt, img_encoder, prompt_encoder, mask_decoder):
        ...
        head_pos, head_neg = cls_prompt_encoder(feature_list[3])  

        return middle_feature, head_pos, head_neg
    train_losses = []
    eval_losses = []
    aucs = []
    epochs = []
    for epoch in range(args.max_epoch):    
        ...
        for idx, (img, seg, flag, label) in enumerate(train_data):
            ...
            with torch.no_grad():
                middle_feature, head_pos, head_neg = model_predict(img_patch,
                                    points_torch,
                                    img_encoder,
                                    prompt_encoder_list,
                                    mask_decoder)
                seg_resize = F.interpolate(seg_patch[0, 0].unsqueeze(0).unsqueeze(0).to(device), scale_factor=middle_feature.shape[2]/patch_size, mode="trilinear")                
                area = torch.sum(seg_resize.view(seg_resize.size(0), -1), dim=1, keepdim=True)
                # positive
                feature_tmp_pos = torch.cat([middle_feature, head_pos], dim=1)
                feature_tmp_pos = torch.mul(feature_tmp_pos, seg_resize)
                feature_tmp_pos = feature_tmp_pos.view(feature_tmp_pos.size(0), feature_tmp_pos.size(1), -1)
                feature_tmp_pos = torch.sum(feature_tmp_pos, axis=2)
                feature_tmp_pos = feature_tmp_pos / area
                # negative
                feature_tmp_neg = torch.cat([middle_feature, head_neg], dim=1)
                feature_tmp_neg = torch.mul(feature_tmp_neg, seg_resize)
                feature_tmp_neg = feature_tmp_neg.view(feature_tmp_neg.size(0), feature_tmp_neg.size(1), -1)
                feature_tmp_neg = torch.sum(feature_tmp_neg, axis=2)
                feature_tmp_neg = feature_tmp_neg / area
                
            pred_label_pos = cls_model(feature_tmp_pos)
            pred_label_neg = cls_model(feature_tmp_neg)            
            
            pred_label = torch.cat([pred_label_neg, pred_label_pos], dim=1)
        ...
        print('loss vl:', epoch, 'acc:', acc, 'auc:', auc, "sen:", sen, "spe:", spe)
        print(conf_mat)

        with open('result.txt', 'a') as file:
             np.savetxt(file, [epoch, acc, auc], fmt='%f', delimiter=',')
